...
export class DatabaseService {
  ...
  async verifySmtp(config: any) {
    return this.request<any>('/verify-smtp', { method: 'POST', body: JSON.stringify(config) });
  }
  ...
  async saveBanner(b: any) { return this.request<any>(`/banners${b.id && !b._isNew ? `/${b.id}` : ''}`, { method: b.id && !b._isNew ? 'PUT' : 'POST', body: JSON.stringify(b) }); }
  ...
  async saveNotice(n: any) { return this.request<any>(`/notices${n.id && !n._isNew ? `/${n.id}` : ''}`, { method: n.id && !n._isNew ? 'PUT' : 'POST', body: JSON.stringify(n) }); }
  async deleteNotice(id: string) { return this.request(`/notices/${id}`, { method: 'DELETE' }); }

  async getOffers() { return this.request<any[]>('/offers'); }
  async saveOffer(o: any) { return this.request<any>(`/offers${o.id && !o._isNew ? `/${o.id}` : ''}`, { method: o.id && !o._isNew ? 'PUT' : 'POST', body: JSON.stringify(o) }); }
  ...
  async saveBespokeService(s: any) { return this.request<any>(`/bespoke-services${s.id && !s._isNew ? `/${s.id}` : ''}`, { method: s.id && !s._isNew ? 'PUT' : 'POST', body: JSON.stringify(s) }); }
  ...
  async savePartner(p: any) { return this.request<any>(`/partners${p.id && !p._isNew ? `/${p.id}` : ''}`, { method: p.id && !p._isNew ? 'PUT' : 'POST', body: JSON.stringify(p) }); }
  ...
  async saveDue(due: any) { return this.request<any>(`/dues${due.id && !due._isNew ? `/${due.id}` : ''}`, { method: due.id && !due._isNew ? 'PUT' : 'POST', body: JSON.stringify(due) }); }
  ...
  async saveMaterialRequest(r: any) { return this.request<any>(`/material-requests${r.id && !r._isNew ? `/${r.id}` : ''}`, { method: r.id && !r._isNew ? 'PUT' : 'POST', body: JSON.stringify(r) }); }

  async getProductRequests() { return this.request<any[]>('/product-requests'); }
  async saveProductRequest(r: any) { return this.request<any>('/product-requests', { method: 'POST', body: JSON.stringify(r) }); }

  async getReviews() { return this.request<any[]>('/reviews'); }
  async saveReview(r: any) { return this.request<any>(`/reviews${r.id && !r._isNew ? `/${r.id}` : ''}`, { method: r.id && !r._isNew ? 'PUT' : 'POST', body: JSON.stringify(r) }); }
