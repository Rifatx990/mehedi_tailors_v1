...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (mode === 'login') {
        /**
         * PRODUCTION SIMULATION: AUTHENTICATION DEFAULTS
         * ------------------------------------------------
        ...
        // Worker Check
        if (formData.email === 'worker@meheditailors.com' && formData.password === 'worker123') {
          const worker = {
        ...
        if (formData.email.includes('fail')) {
          ...
          const user = found || {
            ...
            name: 'Customer', email: formData.email, phone: '+880 17XXX-XXXXXX',
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative"><EnvelopeIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" /><input required type="email" name="email" onChange={handleInputChange} className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-amber-600/20" /></div>
                </div>
                {mode === 'register' && (
                  <div className="space-y-1">
                ...
                {mode !== 'forgot-password' && (
                  <div className="space-y-1">
                    ...
                    <div className="relative"><LockClosedIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" /><input required type="password" name="password" onChange={handleInputChange} className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-amber-600/20" /></div>
                  </div>
                )}
              ...
              <div className="mt-10 text-center text-xs">
                ...
                <div className="mt-6 pt-6 border-t border-slate-100 flex justify-center space-x-4">
                  ...
                  {/* HELPER: Auto-fill worker credentials */}
