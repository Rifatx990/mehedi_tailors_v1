...
const AdminDashboardPage: React.FC = () => {
  const { orders, products, allUsers, emailLogs, systemConfig } = useStore();
  ...
  }), [orders, products, allUsers, emailLogs]);
      ...
      <main className="flex-grow p-4 md:p-16 overflow-y-auto">
        <header className="mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
          ...
          <div className="flex items-center space-x-4 bg-white px-6 py-4 rounded-[1.5rem] border border-slate-100 shadow-sm w-full lg:w-auto justify-between lg:justify-start">
             <div className="flex flex-col text-left lg:text-right">
                <span className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Database Cloud</span>
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">Solid Persistence OK</span>
             </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-16">
          {[
            ...
            { label: 'Outbox Traffic', value: `${stats.totalEmails} Dispatches`, color: 'bg-slate-900', icon: ShieldCheckIcon },
          ].map(stat => (
        ...
        <div className="bg-slate-950 p-8 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] text-white relative overflow-hidden group">
           ...
           <div className="relative z-10">
              ...
              <p className="text-slate-400 max-w-2xl leading-relaxed mb-8 font-light text-sm md:text-base">Your atelier's data is secured via **IndexedDB**, a localized transactional database "file" system. This ensures records are preserved across sessions and hardware power cycles.</p>
              <Link to="/admin/settings" className="bg-white text-slate-900 px-10 py-4 rounded-2xl font-bold uppercase tracking-widest text-[10px] hover:bg-amber-600 hover:text-white transition shadow-2xl inline-block text-center w-full sm:w-auto">Manage DB Files</Link>
           </div>
        </div>
      </main>
    </div>
