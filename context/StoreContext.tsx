...
import { CartItem, Product, User, Order, OrderStatus, Fabric, Coupon, ProductRequest, Banner, Review, ProductionStep, MaterialRequest, EmailConfig } from '../types.ts';
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [emailConfig, setEmailConfig] = useState<EmailConfig>(() => getStored(KEYS.EMAIL_CONFIG, INITIAL_EMAIL_CONFIG));
  ...
  useEffect(() => { sync(KEYS.MATERIALS, materialRequests); }, [materialRequests]);
  useEffect(() => { sync(KEYS.REVIEWS, reviews); }, [reviews]);
  useEffect(() => { sync(KEYS.CATEGORIES, categories); }, [categories]);
  useEffect(() => { sync(KEYS.EMAIL_CONFIG, emailConfig); }, [emailConfig]);
  ...
  const placeOrder = async (order: Order) => {
    ...
    if (emailConfig.isEnabled) {
        console.log(`[SMTP Dispatch] Attempting to send receipt via ${emailConfig.smtpHost} for Order ${order.id}`);
    }
  };
